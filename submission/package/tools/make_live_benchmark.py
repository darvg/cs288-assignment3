...
def answer_within_limit(answer: str, max_words: int = 10) -> bool:
...
def validate_qa_record(record: dict) -> tuple[bool, str]:
    ...
    if not is_allowed_source_url(record["source_url"]):
        return False, f"Source URL outside allowed scope: {record['source_url']}"
    for answer in record["answers"]:
        if not answer_within_limit(answer):
            return False, f"Answer exceeds 10 words: {answer}"
    return True, "ok"
...
def generate_candidates(pages: list[dict]) -> list[dict]:
    ...
    for page in pages:
        ...
        if section in {"resources", "academics", "research", "about", "people", "division", "department", "industry"}:
            ...
            answer = title
            key = f"{question}::{answer}"
            if key not in seen_questions and answer_within_limit(answer):
                candidates.append(
                    {
                        "question": question,
                        "answers": [answer],
